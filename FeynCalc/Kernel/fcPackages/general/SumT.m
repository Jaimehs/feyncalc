...
SumT[1,2,n_Symbol + a_., opt___Rule] := 
 ...
 MakeBoxes[
          ...
          ] := RowBox[{Subsuperscript[OverscripFeynCalc`Tbox["S","~"], 
                    FeynCalc`Tbox[i], n], "(", m , ")"}];
   SumT /:
   MakeBoxes[
             SumT[m_,opt___Rule], TraditionalForm
            ] := RowBox[{OverscripFeynCalc`Tbox["S","~"], 
                         "(", FeynCalc`Tbox[m] , ")"}];

   SumT /:
   MakeBoxes[
             SumT[i_Integer, m_,opt___Rule], TraditionalForm
            ] := RowBox[{SubscripFeynCalc`Tbox[OverscripFeynCalc`Tbox["S","~"], i], 
                         "(", FeynCalc`Tbox[m] , ")"}];

   SumT /:
   MakeBoxes[ SumT[i_Integer,j__Integer, m_,o___Rule], TraditionalForm
            ] := RowBox@@{{SubscripFeynCalc`Tbox[OverscripFeynCalc`Tbox["S","~"], 
