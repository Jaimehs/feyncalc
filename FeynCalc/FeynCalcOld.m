...
FeynCalc`Tbox::"usage"="FeynCalc`Tbox[a, b, ...] produces a RowBox[{a,b, ...}] where \
...
FeynCalc`Tbox[] = "\[Null]";
FeynCalc`Tbox[a_] := ToBoxes[a, TraditionalForm];
FeynCalc`Tbox[]    = "\[Null]";
FeynCalc`Tbox[a_] := totr[a];

FeynCalc`Tbox[a_,b__] := RowBox @ Map[(ToBoxes @@ {#, TraditionalForm})&, {a, b}];
...
FeynCalc`Tbox[a_,b__] := RowBox @ Map[totr, {a,b}];

(*
FeynCalc`Tbox[a_,b__] :=
(RowBox @ (Insert[
